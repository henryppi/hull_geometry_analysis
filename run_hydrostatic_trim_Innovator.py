...
import numpy as np
from my_modules import get_outline_xz_plane, plot3D_line, plot_line_xz, get4x4TransformationRotMat
from my_modules import TriSurface
from my_modules import RigidBody

fn_hull = './CAD/Full_Hull_Innovator.stl'
...
tri.update()
R = np.matrix(np.diag([-1,-1,1]))
print(R)
t = np.array([[0,0,0]]).T
T = get4x4TransformationRotMat(R,t)
tri.move(T)
tri.update()
tri.translate(np.array([-tri.bbox['xmin'],(tri.bbox['ymax']+tri.bbox['ymin'])*0.5,-tri.bbox['zmin']]))
tri.update()
tri.print_size()
...
L,B,T = tri.get_LBT()
mass = 12.5*1e3
cog = np.array([0.34*L,0.5*B,1.378])
tri.set_center(cog)
tri.print_size()
...
x = np.array([[0.0,0.0,0.0]],float)#+np.array([cog])
